...
import { X, GraduationCap, Wallet, FileText, CheckCircle2, XCircle, Clock, History } from "lucide-react";
// ==========================================
// การกำหนด Type
// ==========================================
        ...
        <table className="w-full text-left border-collapse min-w-[900px] bg-white">
          <thead>
            <tr className="bg-gray-100/70 border-b border-gray-300 text-gray-700 text-[14px]">
              ...
              <th className="py-3.5 px-4 font-semibold border-r border-gray-300 min-w-[200px]">รายละเอียดคำร้อง</th>
          ...
          <tbody>
            {requests.map((req, idx) => (
              <tr key={idx} className="border-b border-gray-200 hover:bg-orange-50/20 transition-colors text-[14px]">
                ...
                <td className="py-4 px-4 text-gray-600 border-r border-gray-200 whitespace-nowrap">{req.submitDate}</td>
                <td className="py-4 px-4 text-gray-700 border-r border-gray-200">
                  <div className="line-clamp-2">{req.objective}</div>
                </td>
                <td className="py-4 px-4 text-gray-900 font-bold border-r border-gray-200 whitespace-nowrap">฿{req.amount}</td>
                <td className="py-4 px-4 text-gray-700 border-r border-gray-200 whitespace-nowrap">{req.term} งวด</td>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-[600px] flex flex-col max-h-[90vh] sm:max-h-[85vh] overflow-hidden relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex justify-between items-start px-4 sm:px-6 py-4 border-b border-gray-100 bg-white">
              <div className="pr-2">
                <h2 className="text-lg font-bold text-gray-900 leading-tight">{selectedRequest.name}</h2>
              ...
              <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                ...
                <button onClick={closeAllModals} className="text-gray-400 hover:text-gray-700 bg-gray-50 hover:bg-gray-100 p-1.5 rounded-full transition-colors">
                  <X size={20} />
                </button>
              </div>
            </div>
            ...
            <div className="p-4 sm:p-6 overflow-y-auto flex-1 bg-[#f8fafc] space-y-4">
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-4 shadow-sm">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 text-[14px] sm:text-[15px]">คณะพยาบาลศาสตร์</h3>
                  <p className="text-[13px] text-gray-500 mt-0.5">{selectedRequest.major} · ปี {selectedRequest.year}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4">
                <div className="bg-white border border-gray-200 rounded-xl p-3.5 sm:p-4 shadow-sm">
                  <div className="text-[12px] text-gray-500 flex items-center gap-1.5 mb-1"><Wallet size={14}/> จำนวนที่ขอ</div>
                  <div className="font-bold text-[16px] sm:text-[18px] text-[#ea580c]">฿{selectedRequest.amount}</div>
                </div>
                <div className="bg-white border border-gray-200 rounded-xl p-3.5 sm:p-4 shadow-sm">
                  <div className="text-[12px] text-gray-500 flex items-center gap-1.5 mb-1"><Clock size={14}/> จำนวนงวดที่ผ่อน</div>
                  <div className="font-bold text-[16px] sm:text-[18px] text-gray-900">{selectedRequest.term} งวด</div>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                ...
                <p className="text-[14px] text-gray-800 leading-relaxed">{selectedRequest.objective}</p>
              </div>
                    ...
                    {selectedRequest.history.map((step, index) => (
                      <div key={index} className="relative pl-5">
                        <div className={`absolute w-3 h-3 rounded-full -left-[7px] top-1 ${
                          index === selectedRequest.history.length - 1 ? 'bg-blue-500 ring-4 ring-blue-50' : 'bg-gray-300'
                        }`}></div>
                        <div className={`font-bold text-[14px] ${index === selectedRequest.history.length - 1 ? 'text-gray-900' : 'text-gray-600'}`}>
                          {step.action}
                        </div>
                        <div className="text-[12px] sm:text-[13px] text-gray-500 mt-0.5">{step.date} · {step.actor}</div>
                      </div>
                    ))}
                  </div>
                ) : (
                ...
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200">
                  ...
                  <div className="flex justify-between items-center px-5 py-4 border-b border-gray-100">
                    <h3 className={`font-bold text-[16px] ${confirmAction === 'approve' ? 'text-green-700' : 'text-red-600'}`}>
                      {confirmAction === 'approve' ? 'ยืนยันการอนุมัติ' : 'ระบุเหตุผลในการไม่อนุมัติ'}
                    </h3>
                  ...
                  <div className="p-5">
                    ...
                    <textarea 
                      ...
                      placeholder={confirmAction === 'approve' ? "เช่น นักศึกษามีผลการเรียนดี..." : "เช่น เอกสารรับรองรายได้ไม่สมบูรณ์..."}
                      value={remark}
