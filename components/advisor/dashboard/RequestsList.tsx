...
import RequestsCard, { type ActionRequest } from "@/components/shared/pending/RequestsCard";

export default function RequestsList() {
  const [filter, setFilter] = useState<FilterStatus>("pending");
  const [searchQuery, setSearchQuery] = useState("");
  const requests: ActionRequest[] = mockPendingRequests.map((request) => ({
    ...request,
    phone: "-",
    program: request.major,
  ...
  const filteredRequests = requests.filter((req) => {
    ...
    else if (filter === "cancelled") isStatusMatch = req.requestStatus === "ยกเลิกคำร้อง";
    else if (filter === "pending_admin") isStatusMatch = false;
    else if (filter === "pending_executive") isStatusMatch = false;
