...
// 1. นำเข้า Link จาก Next.js
import Link from 'next/link'; 
import { CalendarCheck, Clock, CheckSquare, X } from 'lucide-react';

// เปลี่ยนชื่อ Function ให้ตรงกับชื่อไฟล์ AdvisorStatCards
export default function AdvisorStatCards() {
  
  // กำหนดเส้นทาง (Path) ของหน้า PendingPage 
  // ตามโครงสร้างโฟลเดอร์ น่าจะเป็น /advisor/pending (หรือปรับเปลี่ยนตาม Routes จริงของคุณ)
  const pendingPagePath = '/advisor/pending';

  // สร้าง Tailwind class สำหรับเอฟเฟกต์ตอน Hover และ Pointer
  const cardLinkClass = "block cursor-pointer hover:scale-102 hover:shadow-md transition-all duration-300";

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
      
      {/* 1. ทั้งหมด (ฟ้า) - หุ้มด้วย Link */}
      <Link href={pendingPagePath} className={cardLinkClass}>
      ...
      {/* 2. รอพิจารณา (เหลือง) - หุ้มด้วย Link */}
      <Link href={pendingPagePath} className={cardLinkClass}>
